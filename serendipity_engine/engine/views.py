    ...
    try:
        ...
        if (user.is_active == False and instance.is_active == True):
            send_mail('Woot! Your Harvard Interop account is live.',
                      ...
                      domain + reverse('django.contrib.auth.views.password_reset')
                      + ' .',
                      settings.FROM_EMAIL,
                      settings.INTEROP_ADMINS)
    
    except User.DoesNotExist:
        # if User.DoesNotExist, we're registering and haven't saved them to db yet
        # no need to do anything; we only send activation emails on the second
        # step of our two-step registration
        pass
pre_save.connect(activate_user, sender=User)
"""TODO:        
  define an activation view that emails the user
  make sure the emails the user gets on registration and activation
  make sense. in particular, no activation key! And remind about username.
